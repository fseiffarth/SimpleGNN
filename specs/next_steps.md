...
### 5. Pre-allocate weight distributions instead of quadratic torch.cat
...
### 6. Pre-allocate bias distributions
...
- **Effort:** 1 hour (same pattern as step 5)
- **Spec:** `01-sharegnn-optimizations.md` Section 3
- **Status:** 🔴 NOT IMPLEMENTED

### 7. Pre-allocate pooling layer distributions
...
### 8. Eliminate per-forward dense matrix allocation
...
### 10. Move evaluation from per-batch to per-epoch
...
### 11. Add configurable validation frequency
...
### 12. Make evaluation batch size configurable
...
### 13. Add semantic validation for config values
...
### 14. Add missing config options (validation_frequency, eval_batch_size, etc.)
...
### 15. Cache NetworkX graph conversions
...
### 16. Parallelize label computation across graphs
...
### 17. Reduce redundant label loading per run
...
### 18. Add ShareGNN batch processing support
...
- **Status:** 🔴 NOT IMPLEMENTED - **HIGHEST IMPACT ITEM**

### 19. Fix tensor shape convention consistency
...
### 20. Add residual projection for dimension mismatches
...
### 21. Make BatchNorm handle 3D input
...
### 22. Add random search / Optuna for hyperparameter optimization
...
### 23. Optimize joblib serialization for parallel runs
...
### 24. Add mixed-precision (AMP) training support
- **Effort:** 4-8 hours
- **Spec:** `04-dataset-and-config-improvements.md` Section 8
- **Status:** 🔴 NOT IMPLEMENTED

### 25. Add schema enforcement for YAML configs
- **Effort:** 4-8 hours
- **Spec:** `04-dataset-and-config-improvements.md` Section 7
- **Status:** 🔴 NOT IMPLEMENTED

### 26. Implement or remove empty positional encoding layer
...
1. **Fix enum checking for backward compatibility** (15 min) - Ensure Python 3.10+ compatibility
2. **Fix type mismatch in property dict check** (10 min) - Critical correctness bug
3. **Replace eval() in graph_dataset.py** (30 min) - Security issue
4. **Cache config lookups in ShareGNN** (5 min) - Quick performance win
5. **Add ShareGNN batch processing** (1-2 days) - **10-50x performance improvement**
