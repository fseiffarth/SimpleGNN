...
# Range Graph Benchmark. See specs/20-lrgb-long-range-benchmark.md.
set -e
...
SPLITS="$ROOT_DIR/src/simplegnn/datasets/splits/fixed/peptides-struct_splits.json"
if [ ! -f "$SPLITS" ]; then
    echo "Split file missing, generating it (downloads Peptides-struct on first run)..."
    python -m simplegnn.utils.lrgb_splits --dataset peptides-struct
fi

# Run the experiment
python experiments/lrgb/main_peptides_struct.py --num_threads "$NUM_THREADS"
